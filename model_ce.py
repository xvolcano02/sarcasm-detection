...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        #把[batchsize,77,768]中每个一个的第一维中的第一个向量拿出来，变成了[batchsize,768]
        first_token_tensor = hidden_states[:, 0]   
        pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(pooled_output)
        return pooled_output
...
class Roberta_vit_qivt_attention(nn.Module):
    def __init__(self,feature_dim=128):
        super(Roberta_vit_qivt_attention, self).__init__()
        self.roberta = RobertaModel.from_pretrained('/data/jingliqiang/xiecan/MILNet/model/roberta-base', return_dict=True)
    ...
    def forward(self,input_mask,bert_mask_add, bert_indices_add, img_trans):
        #文本模态
